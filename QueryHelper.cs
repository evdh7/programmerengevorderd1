...
namespace BedrijvenDataLaag
{
    ...
    public class QueryHelper
    {
        ...
        public List<string> GeefWerknemersMetDezelfdeNaam()
        {
            var res = beheerder.AlleBedrijven
                .SelectMany(p => p.Personeel)
        ...
        public string GeefWerknemersPerLeeftijdCategorie()
        {
            var leeftijdenPersoneel = beheerder.AlleBedrijven.SelectMany(b => b.Personeel);

            int leeftijdMin25 = leeftijdenPersoneel.Count(p => p.DateOfBirth > DateTime.Now.AddYears(-25));

            int leeftijdTussen25en50 = leeftijdenPersoneel.Count(p => ((p.DateOfBirth <= DateTime.Now.AddYears(-25)) &&
                                                                       (p.DateOfBirth > DateTime.Now.AddYears(-50))));

            int leeftijdMeerDan50 = leeftijdenPersoneel.Count(p => p.DateOfBirth <= DateTime.Now.AddYears(-50));


;           return $"Minder dan 25 jaar: {leeftijdMin25} medewerkers, tussen de 25 en 50 jaar {leeftijdTussen25en50} medewerkers, en meer dan 50 jaar {leeftijdMeerDan50} medewerkers";
